...
export function AggregatePlanning({ campaigns, loading, metaCurrency, dv360Currency, dateRange, customStart, customEnd, strictWindow = false }: {
  ...
  <div class="page-body">
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-card"><div class="kpi-lbl">Clicks</div><div class="kpi-val">${fBig(tot.clicks)}</div></div>
      <div class="kpi-card"><div class="kpi-lbl">CTR</div><div class="kpi-val">${tot.imp>0?((tot.clicks/tot.imp)*100).toFixed(2)+"%":"—"}</div></div>
    </div>
    ${campTableBBD(rows, "Campaign Wise Performance", ["Net Spends","Impression","Reach","Freq","eCPM","VTR","CTR"], 20)}
    ${fmtRows.length>0?formatWiseTable(fmtRows, cur):""}
    ${fmtRows.length>0?creativeHeatmapTable(creativeRows, cur):""}
  </div>
  ...
  <div class="page-footer"><span>Generated by Auditor</span><span>${now}</span></div>
    ...
    // Fragments (no page wrapper) — appended INTO the existing pages below,
    // so the deck's page order/count stays exactly as it was, just with
    // charts and a couple more KPIs added where they belong.
    const dailyTrendsFragment = dailySorted.length > 0 ? `
    ${hasDailyReach ? `
    <div class="sec-title" style="margin-top:18px">Reach Build Up</div>
    ...
    ${svgLine([{ name: "Reach", color: "#0072F0", points: reachBuildUp }], dailyTrendLabels, 900, 150)}` : ""}
    <div class="sec-title" style="margin-top:16px">Day-Wise Unique Reach &amp; Impressions</div>
    ${svgLine([{ name: "Daily Unique Reach (Meta)", color: "#0072F0", points: dailySorted.map(d=>d.metaReach) }, { name: "Impressions", color: "#F5A623", points: dailySorted.map(d=>d.impr) }], dailyTrendLabels, 900, 140)}
    <div class="sec-title" style="margin-top:16px">Spend vs Impressions</div>
    ${svgLine([{ name: "Spend", color: "#0072F0", points: dailySorted.map(d=>d.spend) }, { name: "Impressions", color: "#F5A623", points: dailySorted.map(d=>d.impr) }], dailyTrendLabels, 900, 130)}` : "";

    const plannedVsSpendsFragment = weeklyBucketsLocal.length > 0 ? `
    <div class="sec-title" style="margin-top:18px">Overall Planned vs Spends (weekly)</div>
    <div style="font-size:9px;color:#80868b;margin-bottom:8px">${heroPlannedSpend>0?`"Planned" is your total planned spend (${f(heroPlannedSpend,"money",cur0)}) split evenly across ${weeklyBucketsLocal.length} week${weeklyBucketsLocal.length===1?"":"s"} — an even pacing reference, not a per-week target you entered.`:"No planned spend entered yet on the Dashboard tab — showing actual spend only."}</div>
    ${svgGroupedBar(weeklyBucketsLocal.map(w=>({label:w.label, values:[perWeekPlanned, w.spend] as [number, number]})), ["Planned (even pace)","Actual"], ["#0072F0","#F5A623"], (v)=>f(v,"money",cur0), 190)}` : "";
